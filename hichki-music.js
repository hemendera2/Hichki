...
(() => {
  'use strict';
  const audio=document.createElement('audio');audio.preload='metadata';audio.playsInline=true;const state={track:null,playing:false,objectUrl:null};
  ...
  const setTrack=track=>{const nextUrl=String(track?.url||'');if(state.objectUrl&&state.objectUrl!==nextUrl)revoke();state.track={title:String(track?.title||'Untitled'),artist:String(track?.artist||'Unknown'),album:String(track?.album||''),url:nextUrl,cover:String(track?.cover||'')};audio.src=state.track.url;audio.currentTime=0;updateMediaSession();emit('track',state.track)};
  ...
  const play=async()=>{await audio.play();state.playing=true;emit('play',state.track)};const pause=()=>{audio.pause();state.playing=false;emit('pause',state.track)};const toggle=()=>state.playing?pause():play();
