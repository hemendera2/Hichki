...
for (const asset of assets) {
  ...
  if (!source?.isFile() || source.size === 0) throw new Error(`Required public runtime asset is missing or empty: public/${asset}`);
  const destination = `dist/${asset}`;
  const current = await stat(destination).catch(() => null);
  if (!current?.isFile() || current.size !== source.size || current.size === 0) await copyFile(`public/${asset}`, destination);
  const copied = await stat(destination);
  if (!copied.isFile() || copied.size !== source.size || copied.size === 0) throw new Error(`Post-build runtime asset verification failed: ${asset}`);
