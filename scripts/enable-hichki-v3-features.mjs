...
const marker = 'HICHKI_LIBRARY_SOCKET_BRIDGE_V1';
...
if (!html.includes(marker)) {
  html = html.replace('</body>', `<!-- ${marker} --><script src="/hichki-library.js" defer></script></body>`);
}
...
if (!sw.includes('/hichki-library.js')) {
  const anchor = "'/hichki-music.js'";
  if (!sw.includes(anchor)) throw new Error('Unable to extend service-worker runtime asset list');
  sw = sw.replace(anchor, `${anchor},'/hichki-library.js'`);
}
await writeFile(swPath, sw, 'utf8');

console.log(`Hichki library/share target enabled; Socket.IO ${socketUrl ? 'configured' : 'fallback-only'}.`);
