...
for (const asset of assets) {
  ...
  if (!source?.isFile() || source.size === 0) throw new Error(`Required runtime asset is missing or empty: ${asset}`);
  const destination = `public/${asset}`;
  await copyFile(asset, destination);
  const copied = await stat(destination);
  if (!copied.isFile() || copied.size !== source.size || copied.size === 0) throw new Error(`Runtime asset copy verification failed: ${asset}`);
