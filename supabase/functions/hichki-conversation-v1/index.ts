  ...
  try {
    ...
    if (candidateIds.length) {
      ...
      for (const id of candidateIds) {
        ...
        if ((members ?? []).length === 2 && new Set((members ?? []).map((row) => row.user_id)).size === 2) return json({ ok: true, conversation_id: id, created: false });
      }
    }

    const { data: conversation, error: conversationError } = await admin.from("conversations").insert({ kind: "direct", created_by: user.id }).select("id").single();
    if (conversationError || !conversation) throw conversationError ?? new Error("conversation_create_failed");
    const { error: membersError } = await admin.from("conversation_members").insert([{ conversation_id: conversation.id, user_id: user.id }, { conversation_id: conversation.id, user_id: otherUserId }]);
    if (membersError) { await admin.from("conversations").delete().eq("id", conversation.id); throw membersError; }
    return json({ ok: true, conversation_id: conversation.id, created: true });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return json({ ok: false, error: message }, message === "not_authenticated" ? 401 : message === "recipient_not_found" ? 404 : 400);
  }
});
